...
class Supplier extends Model {
    ...
    public function delete($id) {
        try {
            // Tedarikçiyi kullanan alım kayıtlarını kontrol et
            $query = "SELECT COUNT(*) as count FROM purchases WHERE supplier_id = ?";
            $stmt = $this->db->prepare($query);
            $stmt->bindParam(1, $id);
            $stmt->execute();
            ...
            if($result['count'] > 0) {
                return false; // Tedarikçiyi kullanan alım kayıtları varsa silme
            }
    ...
    public function generateSupplierCode() {
        try {
            $query = "SELECT MAX(CAST(SUBSTRING(code, 2) AS UNSIGNED)) as max_code FROM " . $this->table . " WHERE code LIKE 'S%'";
            $stmt = $this->db->prepare($query);
            $stmt->execute();
            $result = $stmt->fetch(PDO::FETCH_ASSOC);
            
            $next_code = (int)$result['max_code'] + 1;
            return 'S' . str_pad($next_code, 5, '0', STR_PAD_LEFT);
        } catch (PDOException $e) {
            return 'S00001'; // Varsayılan ilk kod
        }
    }
}